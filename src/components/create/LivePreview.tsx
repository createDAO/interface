...
export const LivePreview: React.FC<LivePreviewProps> = ({ formData, networkName }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 sticky top-6">
      ...
      {/* DAO Name Preview */}
      <div className="mb-6">
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">{t('preview.daoNameLabel')}</p>
        <p className="text-xl font-bold text-gray-900 dark:text-white">
          {formData.daoName || t('preview.daoNamePlaceholder')}
        </p>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          {t('preview.onNetwork', { network: networkName })}
        </p>
      </div>

      {/* Token Info */}
      <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">{t('preview.governanceTokenLabel')}</p>
        <div className="flex items-baseline gap-2">
          <span className="text-lg font-semibold text-gray-900 dark:text-white">
            {formData.tokenName || t('preview.tokenNamePlaceholder')}
          </span>
          <span className="text-sm text-gray-500 dark:text-gray-400">
            ({formData.symbol || t('preview.tokenSymbolPlaceholder')})
          </span>
        </div>
        {totalSupply > 0 && (
          <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">
            {t('preview.totalSupplyLine', { amount: totalSupply.toLocaleString() })}
          </p>
        )}
      </div>
      ...
      {totalSupply > 0 && (
        <div className="mb-6">
          <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">{t('preview.tokenDistribution.title')}</p>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <span className="text-sm text-gray-600 dark:text-gray-300">{t('preview.tokenDistribution.creatorLabel')}</span>
              </div>
              <span className="text-sm font-medium text-gray-900 dark:text-white">
                {t('preview.tokenDistribution.creatorAmount', { amount: creatorAmount.toLocaleString() })}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <div className="w-3 h-3 rounded-full bg-amber-500 mr-2" />
                <span className="text-sm text-gray-600 dark:text-gray-300">{t('preview.tokenDistribution.treasuryLabel')}</span>
              </div>
              <span className="text-sm font-medium text-gray-900 dark:text-white">
                {t('preview.tokenDistribution.treasuryAmount', { amount: treasuryAmount.toLocaleString() })}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
        <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">{t('preview.governance.title')}</p>
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-gray-900 dark:text-white">{t('preview.governance.quorumValue')}</span>
          </div>
          <div className="flex justify-between">
      ...
      <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
        <p className="text-xs text-blue-700 dark:text-blue-300">
          <strong>{t('preview.tip.title')}:</strong> {t('preview.tip.text')}
        </p>
      </div>
    </div>
  );
